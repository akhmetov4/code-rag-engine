...
def _run_process(
    ...
    try:
        ...
        logger.exception("Process failed: job_id=%s", job_id)
        job["status"] = "error"
        job["error"] = str(exc)
...
def process_project(
    ...
    thread = threading.Thread(
        target=_run_process,
        args=(job_id, req.path, req.workers, req.chunk_size),
        daemon=True,
    )
    thread.start()
...
def ask_global(req: GlobalAskRequest) -> Dict[str, Any]:
    """Full RAG across ALL projects listed in PROJECT_ROOTS.

    Loads passports from disk for every project root, runs semantic search
    over all of them, and generates a single grounded answer.
    """
    if not PROJECT_ROOTS:
        raise HTTPException(
            status_code=500,
            detail="PROJECT_ROOTS is empty. Set it in .env.",
        )

    # 1. Collect passports from every configured project root
    project_names: List[str] = []
    passports_by_path: Dict[str, Any] = {}

    for root_path in PROJECT_ROOTS:
        pname = _derive_project_name(str(root_path))
    ...
    if not project_names:
        raise HTTPException(
            status_code=400,
            detail="No indexed passports found on disk. Run `python main.py process` first.",
        )

    # 2. Build effective query (incorporate rules if provided)
    ...
    # 3. Translate → search → expand → generate
    english_query = translate_to_english(req.query)
    logger.info(
        "[ask_global] original=%r  english=%r  projects=%s",
        req.query, english_query, project_names,
    )
